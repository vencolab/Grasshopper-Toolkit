...
#region Members
  ...
  /// </summary>
  private void RunScript(List<Curve> crvs, Plane pts, ref object A, ref object B, ref object C)
  {
    ...
    {
      crv = crvs[i].Extend(CurveEnd.Both, crvs[i].GetLength(), CurveExtensionStyle.Line);
      newCrvs.Add(crv);

    }

    var crvRegions = Curve.CreateBooleanRegions(newCrvs, pts, true, 0.01);

    A = crvRegions.RegionCurves(0);

    B = crvRegions.RegionCount;
    C = newCrvs;
  ...
  public override void InvokeRunScript(IGH_Component owner, object rhinoDocument, int iteration, List<object> inputs, IGH_DataAccess DA)
  {
    ...
    if (inputs[0] != null)
    {
      crvs = GH_DirtyCaster.CastToList<Curve>(inputs[0]);
    }
    Plane pts = default(Plane);
    if (inputs[1] != null)
    {
      pts = (Plane)(inputs[1]);
    }
  ...
  object C = null;
    ...
    //4. Invoke RunScript
    RunScript(crvs, pts, ref A, ref B, ref C);
